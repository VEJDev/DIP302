<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email']) && !empty($_POST['password'])) {
		...
		$stmt = $conn->prepare("SELECT id, password FROM profils WHERE email = ?");
		...
		if ($stmt->num_rows > 0) {
			...
			if (password_verify($password, $hashedPassword)) {
				...
				$_SESSION['message'] = "Nepareizs epasts/parole";
				header("Location: auth.php");
			}
		} else {
			$_SESSION['message'] = "Lietotājs netika atrasts";
			header("Location: auth.php");
		}
	...
	<body>
		...
		<div id="content" style="display: flex; flex-direction: column; align-items: center;">
			<div class="auth-container">
				...
				<p>Nav Konta? <a href="auth2.php">Izveidot jaunu kontu</a></p>
				<p style="margin-top: 0px;">Aizmirsi paroli? <a href="auth3.php">Atjaunot paroli</a></p>
			</div>
		</div>
		<footer>

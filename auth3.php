<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email'])) {
		...
		$stmt = $conn->prepare("SELECT id FROM profils WHERE email = ?");
		...
		if ($stmt->num_rows > 0) {
			...
			$stmt2 = $conn->prepare("UPDATE profils SET pw_reset_token= ? WHERE email = ?");
			$stmt2->bind_param("ss", $token, $email);
			$stmt2->execute();

			$subject = "Paroles atjaunošana";
			$message = "Ja Jūs nosūtījāt paroles atjaunināšanas pieprasījumu, tad spiediet uz šīs saites: http://127.0.0.1/auth4.php?token=$token";

			$headers = "From: no-reply@presesapvieniba.lv\r\n";
		...
		$_SESSION['message'] = "Ja šāds e-pasts ir datu bāzē, tad e-pasts tika nosūtīts.";
		header("Location: index.php");
	...
	<body>
		...
		<div id="content" style="display: flex; flex-direction: column; align-items: center;">
			<div class="auth-container">
				...
				<p>Uz e-pastu tiks nosūtīta saite, kuru nospiežot, Jūs varēsiet atjaunot paroli.</p>
			</div>
		</div>
		<footer>

<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'GET' && !empty($_GET['token'])) {
		...
		$stmt = $conn->prepare("SELECT id FROM profils WHERE pw_reset_token = ?");
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['password']) && !empty($_POST['token'])) {
		$password = $_POST['password'];
		$token = $_POST['token'];
		$password = password_hash($_POST['password'], PASSWORD_DEFAULT);

		$stmt2 = $conn->prepare("UPDATE profils SET pw_reset_token=NULL, password=? WHERE pw_reset_token=?");
			...
			<div class="auth-container">
				<?php
					if (empty($token) || empty($user_id)) {
						echo "<h2 style=\"text-align: center;\">Paroles atjaunošanas pieprasījums netika atrasts.<h2>";
					} else {
						$html = <<<HTML
						...
						<form class="auth-form" id="authForm" style="display: flex; flex-direction: column; justify-content: center;" method="POST">
							...
							<br>
							<button type="submit" style="margin-bottom: 20px;">Atjaunot Paroli</button>
						</form>
						HTML;
						echo $html;
					}

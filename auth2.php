<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email']) && !empty($_POST['email'])) {
		$username = htmlspecialchars($_POST['username']);
		if (!preg_match('/^[a-zA-Z0-9_]+$/', $username)) {
			$_SESSION['message'] = "Reģistrācijas kļūda, pārbaudiet rakstu zīmes. (Drīkst būt: burti, cipari un apakšsvītra)";
		...
		$stmt = $conn->prepare("INSERT INTO profils (username, email, password, level, created) VALUES (?, ?, ?, 1, NOW());");
		$stmt->bind_param("sss", $username, $email, $password); // Priekš SQL Injekcijas prevencijas
		...
		try {
			...
			$_SESSION['message'] = "Reģistrācijas kļūda, pārbaudiet rakstu zīmes, lietotājvārda garumu, u.c. Iespējams, ka šāds lietotājs jau eksistē.";
			header("Location: auth2.php");
		}
			...
			<div class="auth-container">
				...
				<form class="auth-form" id="authForm" style="display: flex; flex-direction: column; justify-content: center;" method="POST">
					<div style="display: flex; flex-direction: column; justify-content: center;">
						...
						<input type="text" id="username" name="username" placeholder="Ievadiet lietotājvārdu" required>
						<label for="email">E-pasts</label>
						<input type="email" id="email" name="email"  placeholder="Ievadiet e-pastu" required>
						<label for="password">Parole</label>
						<input type="password" id="password" name="password" placeholder="Ievadiet paroli" required>
					</div>
					<br>
					<button type="submit">Izveidot kontu</button>
				</form>
				<p>Ir Konts? <a href="auth.php">Pierakstīties</a></p>
			</div>
		</div>
		<footer>
